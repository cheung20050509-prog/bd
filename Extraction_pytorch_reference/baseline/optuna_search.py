...
def suggest_train_args(trial, base_args):
    ...
    tokens_per_column = trial.suggest_categorical('tokens_per_column', [8, 16, 32])
    pair_head = trial.suggest_categorical('pair_head', ['concat', 'interaction'])
    ...
    args.tokens_per_column = tokens_per_column
    args.pair_head = pair_head
    args.run_name = f'doduo_trial_{trial.number:04d}'
...
def train_args_from_trial_params(params, base_args, run_name='doduo_best'):
    args = argparse.Namespace(**vars(base_args))
...
def build_optuna_parser():
    ...
    parser.add_argument('--study_name', type=str, default='doduo_pair_deberta_score_final')
    parser.add_argument(
        '--storage',
        type=str,
        default='sqlite:///./optuna_output/doduo_pair_optuna.db',
    )
    ...
    parser.add_argument(
        '--train-best',
        action='store_true',
        help='Retrain with best trial hyperparameters (run_name=doduo_best)',
    )
    parser.add_argument(
        '--infer-after-best',
        action='store_true',
    ...
    parser.add_argument(
        '--infer-output',
        type=str,
        default='../../nlp/submission_doduo_reference.csv',
    )
    return parser
